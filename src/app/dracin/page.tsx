...
// List Genre yang akurat sesuai data API Sansekai
...
export default function DracinPage() {
  ...
  const fetchMovies = useCallback(async () => {
    ...
    // LOGIC: Jika ada genre selain "Semua", prioritaskan pencarian berdasarkan genre tersebut
    const searchTerm = selectedGenre !== "Semua" ? selectedGenre : query;

    if (searchTerm) {
      url = `https://api.sansekai.my.id/api/flickreels/search?query=${encodeURIComponent(searchTerm)}`;
    } else {
      switch (activeTab) {
        case "HOT": url = "https://api.sansekai.my.id/api/flickreels/hotrank"; break;
        case "FORYOU": url = "https://api.sansekai.my.id/api/flickreels/foryou"; break;
        case "LATEST": 
        default: url = "https://api.sansekai.my.id/api/flickreels/latest"; break;
      }
    }
    ...
    try {
      ...
      const normalized: Movie[] = rawData.map((item: any) => {
        let tag = "Drama";
        if (item.playlet_tag_name && item.playlet_tag_name.length > 0) tag = item.playlet_tag_name[0];
        else if (item.tag_list && item.tag_list.length > 0) tag = item.tag_list[0].tag_name;
        else if (item.tag_name && Array.isArray(item.tag_name)) tag = item.tag_name[0];
      ...
      <section className="relative z-10 max-w-7xl mx-auto px-4 md:px-8">
        ...
        {loading ? (
           <div className="flex flex-col items-center justify-center py-20 opacity-50">
              <IconLoading />
              <p className="mt-4 font-black uppercase text-[10px] tracking-widest">Waking up the projectionist...</p>
           </div>
        ) : (
           <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {movies.map((movie) => (
                <div key={movie.id} className="group relative bg-white border-[3px] border-[#0F172A] rounded-[20px] overflow-hidden shadow-[6px_6px_0px_#0F172A] hover:-translate-y-[4px] hover:shadow-[10px_10px_0px_#0F172A] transition-all duration-300">
        ...
        {!loading && movies.length === 0 && (
            <div className="mt-10 bg-white border-[3px] border-[#0F172A] rounded-[20px] p-12 shadow-[10px_10px_0px_#FF99C8] text-center max-w-lg mx-auto">
                ...
                <h3 className="text-2xl font-black uppercase mb-2 leading-none">The beach is empty</h3>
                <p className="text-[10px] font-bold opacity-60 uppercase tracking-widest">
                    Coba cari judul drama atau genre lain, Bre.
                </p>
            </div>
        )}
      </section>
