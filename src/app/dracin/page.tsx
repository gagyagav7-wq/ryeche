...
export default function DracinPage() {
  ...
  const fetchMovies = useCallback(async () => {
    ...
    // Tentukan URL berdasarkan Tab / Search
    if (query) {
    ...
    try {
      ...
      // --- 2. NORMALISASI DATA (Karena struktur API beda-beda) ---
      if (query) {
         // Search: data langsung array
         rawData = json.data || [];
      } else if (activeTab === "HOT") {
         // Hotrank: data[0].data
         rawData = json.data?.[0]?.data || [];
      } else if (activeTab === "LATEST") {
         // Latest: data[0].list
         rawData = json.data?.[0]?.list || [];
      } else if (activeTab === "FORYOU") {
         // Foryou: data.list
         rawData = json.data?.list || [];
      }

      // Mapping ke Format UI Kita
      const normalized: Movie[] = rawData.map((item: any) => {
        // Cari Tag yang paling relevan
        let tag = "Drama";
        ...
        return {
          id: item.playlet_id || item.id, // ID kadang playlet_id, kadang id
          title: item.title,
          cover: item.cover,
      ...
      setMovies([]); // Kosongkan kalau error
    } finally {
      setLoading(false);
    }
  }, [activeTab, query]);

  // Panggil API tiap ganti tab atau search (dengan debounce dikit via useEffect)
  useEffect(() => {
      ...
      <section className="relative z-10 max-w-7xl mx-auto px-4 md:px-8">
        ...
        <div className="flex items-end gap-3 mb-6 border-b-[3px] border-[#0F172A] pb-4">
            ...
            <p className="text-xs font-bold opacity-50 mb-1">
                {loading ? "FETCHING..." : `SHOWING ${movies.length} TITLES`}
            </p>
        </div>
        ...
        {loading ? (
           <div className="flex flex-col items-center justify-center py-20 opacity-50">
              <IconLoading />
              <p className="mt-4 font-black uppercase text-xs tracking-widest">Loading Library...</p>
           </div>
        ) : (
            ...
            {movies.map((movie) => (
                <div key={movie.id} className="group relative bg-white border-[3px] border-[#0F172A] rounded-[16px] overflow-hidden shadow-[4px_4px_0px_#0F172A] hover:-translate-y-[4px] hover:shadow-[8px_8px_0px_#0F172A] transition-all duration-300">
                    ...
                    <div className="aspect-[3/4] bg-gray-200 relative overflow-hidden border-b-[3px] border-[#0F172A]">
                        {/* Pakai unoptimized dulu biar aman kalau domainnya aneh2, atau config di next.config */}
                        <Image 
                            src={movie.cover}
                            alt={movie.title}
                            fill
                            ...
                            unoptimized={true} // Opsional: kalau gambar sering error, nyalakan ini
                        />
        ...
        {!loading && movies.length === 0 && (
            <div className="mt-10 bg-white border-[3px] border-[#0F172A] rounded-[16px] p-8 md:p-12 shadow-[6px_6px_0px_#0F172A] text-center max-w-lg mx-auto">
                ...
                <h3 className="text-2xl font-black uppercase mb-2">Tidak ada data</h3>
                <p className="text-sm font-bold opacity-60">
                    Coba ganti kategori tab atau kata kunci pencarian.
                </p>
            </div>
        )}
