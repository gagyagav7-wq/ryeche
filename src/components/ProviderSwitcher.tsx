...
export default function ProviderSwitcher() {
  ...
  // Logic Active State: Mapping rute ke ID server
  const getActive = (path: string) => {
    if (path.startsWith("/dracin")) return "flickreels";
    if (path.startsWith("/dramabox")) return "dramabox";
    return "";
  };
  ...
  return (
    <div className="flex items-center gap-2 bg-white border-[3px] border-[#0F172A] rounded-full p-1.5 shadow-[5px_5px_0px_#0F172A] mb-0 w-fit">
      {servers.map((srv) => {
        ...
        if (!srv.ready) {
          return (
            <div key={srv.id} className="flex items-center gap-2 px-4 py-2 opacity-40 cursor-not-allowed grayscale">
              ...
              <span className="text-[10px] font-bold uppercase hidden md:inline">{srv.label}</span>
            </div>
          );
        }
        ...
        return (
          ...
          >
            ...
            <span className="hidden md:inline">{srv.label}</span>
          </Link>
        );
      })}
    </div>
