...
const targetPath = path.join(__dirname, 'src/app/dracin/page.tsx');

const cleanCode = `import Image from "next/image";
...
// --- SECTION COMPONENT (Sekarang dengan Tipe Data TypeScript!) ---
const DramaSection = ({ title, subtitle, items, href }: { title: string; subtitle: string; items: any[]; href: string }) => {
...
console.log("Writing cleaner code with TYPES to: " + targetPath);
fs.writeFileSync(targetPath, cleanCode, { encoding: 'utf8' });
console.log("SUCCESS! File has been rewritten cleanly.");
